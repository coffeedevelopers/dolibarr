...
class pdf_afipservice_fe extends ModelePDFFactures
{
	...
	function __construct($db)
	{
        ...
        //cargo el CSV
        $rows = array_map('str_getcsv', file($conf->afipservice->dir_output.'/'.$conf->entity.'/wsfev1/'.$conf->global->AFIPSERVICE_WSFE_PDF_CSV));
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
               ...
               //Cargo Plantilla
				//$pagecount = $pdf->setSourceFile(DOL_DOCUMENT_ROOT.'/afipservice/plantillas/factura.pdf');

                $pagecount = $pdf->setSourceFile($conf->afipservice->dir_output.'/'.$conf->entity.'/wsfev1/'.$conf->global->AFIPSERVICE_WSFE_PDF_TEMPLATE);
