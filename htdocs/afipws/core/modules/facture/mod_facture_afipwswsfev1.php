...
/**
 *	\file       htdocs/core/modules/facture/mod_facture_afipwsfev1.php
 *	\ingroup    facture
...
/**
 *	\class      mod_facture_afipwswsfev1
 *	\brief      Clase de modelo de numeracion para Factuea Electronica
 */
class mod_facture_afipwswsfev1 extends ModeleNumRefFactures
{
     ...
     */
    function info($langs=null) //?CAMBIO TOMAS -> function info() - function info($langs) 
    {
    ...
    function getNextValue($objsoc,$facture,$mode='next')
    {
      ...
      //WSFE  
      	   if ($facture->model_pdf == 'afipws_fe' && $facture->mode_reglement_code!="ECO" && $mode=="next")   //agregar ExtraField en fatura llamado fe
           { 

               // Obtener el �ltimo n�mero para este tipo de comprobante / punto de venta:
               require_once DOL_DOCUMENT_ROOT .'/afipws/lib/wsfev1.lib.php';

               if ($_SESSION['wsfereproceso'][$facture->ref]) {
                $nro=$_SESSION['wsfereproceso'][$facture->ref];
               }else {
                   $nro = get_CompNextNum($facture);  //asigno el numero de Factura
               }
               ...
               if ($facture->mode_reglement_code=="ECO") {
                   if ($facture->type == 1) $mask="FX-0001-{0000000}";
                   else if ($facture->type == 2) $mask="NCX-0001-{0000000}";
                   else if ($facture->type == 3) $mask="NDX-0001-{0000000}";
                   else if ($facture->type == 201) $mask="FCEX-0001-{0000000}";
                   else if ($facture->type == 202) $mask="NCEX-0001-{0000000}";
                   else if ($facture->type == 203) $mask="NDEX-0001-{0000000}";
                   else  $mask="FX-0001-{0000000}";
