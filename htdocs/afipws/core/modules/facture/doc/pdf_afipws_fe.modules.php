...
class pdf_afipws_fe extends ModelePDFFactures
{
	...
	function __construct($db)
	{
		...
		//$rows = array_map('str_getcsv', file(DOL_DOCUMENT_ROOT.'/afipws/plantillas/factura.csv'));
        //cargo el CSV
        $rows = array_map('str_getcsv', file($conf->afipws->dir_output.'/'.$conf->entity.'/wsfev1/'.$conf->global->AFIPWS_WSFE_PDF_CSV));
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
               ...
               //Cargo Plantilla
				//$pagecount = $pdf->setSourceFile(DOL_DOCUMENT_ROOT.'/afipws/plantillas/factura.pdf');

                $pagecount = $pdf->setSourceFile($conf->afipws->dir_output.'/'.$conf->entity.'/wsfev1/'.$conf->global->AFIPWS_WSFE_PDF_TEMPLATE);
				...
				$pdf->SetCreator("Dolibarr ".DOL_VERSION." afipws_2.0");
...
//		}
		...
		$pdf->Image(DOL_DOCUMENT_ROOT.'/afipws/img/afip.png',$posx,	$posy, 27, 7.5);
