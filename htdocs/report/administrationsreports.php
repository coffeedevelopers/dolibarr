 ...
 *		\ingroup    report othermodule1 othermodule2
...
dol_include_once('/report/class/skeleton_class.class.php');
...
// Security check
if (empty($user->rights->report->suppliers)) $result = restrictedArea($user,'report', $socid);
...
if (empty($reshook))
{
	...
	if ($action == 'add')
	{
		if (GETPOST('cancel'))
		{
			$urltogo=$backtopage?$backtopage:dol_buildpath('/report/list.php',1);
...
//		$object->prop2=GETPOST("field2");
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/report/list.php',1));
			exit;
		}
		else
		{
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("Newreport"));
...
if ($id && (empty($action) || $action == 'view' || $action == 'delete'))
{
	...
	if (empty($reshook))
	{
		if ($user->rights->report->write)
		{
			print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&amp;action=edit">'.$langs->trans("Modify").'</a></div>'."\n";
		}

		if ($user->rights->report->delete)
		{
...
$q->initial_project = "administrationsreports";
