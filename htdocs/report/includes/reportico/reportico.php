...
//                    ( ( array_key_exists($col->query_name, $_REQUEST) && !(is_array($_REQUEST[$col->query_name]) && count($col->query_name) == 1 && $_REQUEST[$col->query_name][0] == "" )) //CATRIEL
                ( ( array_key_exists($col->query_name, $_REQUEST) && !(is_array($_REQUEST[$col->query_name]) &&  $_REQUEST[$col->query_name][0] == "" )) //CATRIEL
			...
			if ( array_key_exists($col->query_name, $_REQUEST) )
			{
                // Since using Select2, we find unselected list boxes still send an empty array with a single character which we dont want to include
                // as a criteria selection
//                if ( !(is_array($_REQUEST[$col->query_name]) && count($col->query_name) == 1 && $_REQUEST[$col->query_name][0] == "") ) //CATRIEL
	...
	function & build_where_list($include_lookups = true)
	{
        ...
        //$str = " \nWHERE 1 = 1 ".$this->where_text; //CATRIEL
		else
			$str = " \nWHERE 1 = 1 ";
...
//        $smarty->template_dir =  dol_buildpath("/reports/includes/reportico/templates", 0); //catriel
//        $smarty->compile_dir = dol_buildpath("/reports/includes/reportico/templates_c", 0); //catriel
...
//echo "set table ";
        ...
        if (isset ($user->rights->report->report->write)){
           set_reportico_session_param("admin_password","1"); //CATRIEL
        }else{
            unset_reportico_session_param('admin_password');
        }
...
function set_project_environment($initial_project = false, $project_folder = "projects", $admin_project_folder = "projects" )
{
	...
	$language = "en_gb";
    // Default language to first lan8guage in avaible_languages
