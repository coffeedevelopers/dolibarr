...
/**
 * \file    test/phpunit/HooksPrimetecFunctionalTest.php
...
/**
 * Class HooksPrimetecFunctionalTest
 *
 ...
 *
 * @package Testhooksprimetec
 */
class HooksPrimetecFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modHooksPrimetec')->click();
		} else {
			// Disable the module
			$this->byHref('modHooksPrimetec')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/hooksprimetec/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('hooksprimetec/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/hooksprimetec/admin/about.php');
		$this->authenticate();
		return $this->assertContains('hooksprimetec/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('hooksprimetecwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modHooksPrimetec_HooksPrimetecTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modHooksPrimetec_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
