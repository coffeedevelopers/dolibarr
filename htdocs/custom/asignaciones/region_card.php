...
/**
 *   	\file       region_card.php
 *		\ingroup    operaciones
 *		\brief      Page to create/edit/view region
 */
...
dol_include_once('/operaciones/class/region.class.php');
dol_include_once('/operaciones/lib/operaciones_region.lib.php');
...
$contextpage= GETPOST('contextpage', 'aZ')?GETPOST('contextpage', 'aZ'):'regioncard';   // To manage different context of search
$backtopage = GETPOST('backtopage', 'alpha');
//$lineid   = GETPOST('lineid', 'int');

// Initialize technical objects
$object=new Region($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->operaciones->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('regioncard','globalcard'));     // Note that conf->hooks_modules contains array
...
//$isdraft = (($object->statut == Region::STATUS_DRAFT) ? 1 : 0);
...
if (empty($reshook))
{
    ...
    $backurlforlist = dol_buildpath('/operaciones/region_list.php', 1);
    if (empty($backtopage)) {
        if (empty($id)) $backtopage = $backurlforlist;
        else $backtopage = dol_buildpath('/operaciones/region_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
    }
    $triggermodname = 'OPERACIONES_REGION_MODIFY';	// Name of trigger action code to execute when we modify record
    ...
    $trigger_name='REGION_SENTBYMAIL';
    $autocopy='MAIN_MAIL_AUTOCOPY_REGION_TO';
    $trackid='region'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Region")));
...
{
	print load_fiche_titre($langs->trans("Region"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("Region"), -1, $object->picto);
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteRegion'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action == 'clone') {
		...
		$formquestion = array();
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneRegion', $object->ref), 'confirm_clone', $formquestion, 'yes', 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/operaciones/region_list.php', 1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('region'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/operaciones/region_agenda.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'region', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
