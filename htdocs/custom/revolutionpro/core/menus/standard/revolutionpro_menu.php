<?php
/* Copyright (C) 2005-2013 Laurent Destailleur  <revolutionpro@users.sourceforge.net>
...
/**
 *  \file       htdocs/core/menus/standard/revolutionpro_menu.php
 *  \brief      Menu revolutionpro manager
 */


/**
 *  Class to manage menu REVOLUTIONPRO
 */
class MenuManager
{
    /**
    ...
    public function loadMenu($forcemainmenu = '', $forceleftmenu = '')
    {
        ...
        $tabMenu=array();
        $menuArbo = new Menubase($this->db, 'revolutionpro');
        $menuArbo->menuLoad($mainmenu, $leftmenu, $this->type_user, 'revolutionpro', $tabMenu);
    ...
    public function showmenu($mode, $moredata = null)
    {
        ...
        if($mode == 'topnb') return 0;

        $mode = 'top';
        
        require_once 'revolutionpro.lib.php';
        ...
        if (empty($conf->dol_hide_leftmenu) && (!defined('NOREQUIREMENU')))
            print_revolutionpro_menu($this->db, $this->atarget, $this->type_user, $this->tabMenu, $this->menu, 0, $mode);
        
        $conf->dol_hide_leftmenu = 1;

        $object = new revolutionprocls($this->db);
        $result = $object->fetch();
